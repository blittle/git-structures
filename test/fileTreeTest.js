...
describe('Tree structure', function() {
    ...
    describe('Tree history', function() {
        var history;

        beforeEach(function() {
            history = FileTree.fileTreeHistory(parsedData);
        });

        it('File history should be the same length of commit history', function() {
            assert.equal(parsedData.length, history.length);
        });

        it('Should update changes', function() {
            assert.equal(3, history[6].children[2].size);
            assert.equal(4, history[7].children[2].size);
        })
